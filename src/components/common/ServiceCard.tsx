  ...
  return (
    <Link href={`/services/${serviceId}`}>
      <Card className={`overflow-hidden border-none shadow-none group cursor-pointer transition-all bg-transparent ${className}`}>
        <div className="relative aspect-4/3 rounded-md overflow-hidden mb-3 border border-slate-100 shadow-sm">
          <Image
            src={image}
            alt={title}
            fill
        ...
        <CardContent className="p-0 space-y-2">
          <div className="flex flex-col gap-1">
            <div className="flex items-center justify-between gap-2">
              <h3 className="font-semibold text-base text-slate-900 group-hover:text-[#006767] transition-colors line-clamp-1 leading-snug">
                {title}
              </h3>
            </div>
            ...
            <div className="mt-1 flex items-baseline gap-1">
              <span className="text-[#f97316] font-bold text-base">Rs. {price.toLocaleString()}</span>
              <span className="text-slate-500 text-sm font-medium"> / {unit}</span>
            </div>

            <p className="text-sm font-medium line-clamp-2 leading-relaxed mt-1 ">
              {description}
            </p>
          </div>
        </CardContent>
